...
class DynamicSignalConv2D(tfc.SignalConv2D):
  def __init__(self, **kwargs):
    super(DynamicSignalConv2D, self).__init__(**kwargs)
    self.active_filters = self.filters
  ...
  def call(self, inputs):
    ...
    kernel = kernel[...,...,...,:self.active_filters]
    ...
    if self.use_bias:
      bias = self.bias[:self.active_filters]
  ...
  def sort_filter(self):
    ...
    sorted_importance, sorted_idx = tf.sort(importance, direction='DESCENDING')
    self.kernel = tf.gather(self.kernel, sorted_idx, axis=3)
