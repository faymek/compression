...
def test_compress(args):
  ...
  with tf.Session() as sess:
    ...
    f = open("f7.csv", "w")
    print("level,fn,dbpp,bpp,mse,np", file=f)
    for filename in glob.glob("kodak/*.png"):
      buff_y = 0 #value
      buff_z = 0
      for i in range(0,8):

        side_string = entropy_bottleneck.compress(z - buff_z)
        string = conditional_bottleneck.compress(y - buff_y)

        tensors = [string, side_string,
                    x_shape[1:-1], y_shape[1:-1], z_shape[1:-1]]

        arrays, by, bz, v_lmbda_level, v_eval_bpp, v_mse, v_num_pixels = sess.run(
            [tensors, y, z, lmbda_level, eval_bpp, mse, num_pixels], feed_dict={fn: filename, lmbda_level: i})
        ...
        print("%.2f, %s, %.4f, %.4f, %.4f, %d"%(v_lmbda_level, filename, bpp, v_eval_bpp, v_mse, v_num_pixels), file=f)
    f.close()  
