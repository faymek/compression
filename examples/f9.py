...
class SynthesisTransform(tf.keras.layers.Layer):
  ...
  def __init__(self, num_filters, lmbda, actives, *args, **kwargs):
    self.num_filters = num_filters
    self.lmbda = lmbda
  ...
  def call(self, tensor):
    for layer, active in zip(self._layers, self.actives):
      tensor = layer(tensor, self.lmbda, active)
    return tensor
...
def test_train(args):
  ...
  with tf.Session() as sess:
    ...
    # shapes.
    latest = tf.train.latest_checkpoint(checkpoint_dir="f4")
    tf.train.Saver().restore(sess, save_path=latest)

  actives = [tf.random_uniform([], minval=4, maxval = 33, dtype=tf.int32) * 8 for i in range(3)]
  actives.append(tf.constant(3, dtype=tf.int32))
  ...
  tf.summary.scalar("active1", actives[1])
...
def test_compress(args):
  ...
  with tf.Session() as sess:
    ...
    ac_list = [
      (88,128,64),(88,128,96),(88,168,96),(88,256,64),(88,256,96),(256,256,256)
    ]
    ...
    f = open("f9.csv", "w")
    print("c0,c1,c2,level,bpp,mse", file=f)
    for acs in ac_list:
      print(acs)
      for i in np.arange(0,8):
        count_bpp = 0
        ...
        for filename in glob.glob("kodak/*.png"):
          ...
          print("%d, %d, %d, %d, %.4f, %.4f, %s"%(acs[0], acs[1], acs[2], v_lmbda_level, v_eval_bpp, v_mse, filename), file=f)
